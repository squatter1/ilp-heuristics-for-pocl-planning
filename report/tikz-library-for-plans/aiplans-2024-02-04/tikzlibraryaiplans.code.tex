...
% #3 tikz edge/arrow
\newcommand{\link}[3]{
  \node (aiplans circle from) at (#1) {};
  \node (aiplans circle to)   at (#2) {};
  \draw (aiplans circle from) #3 (aiplans circle to);
}

% A version of link with the \draw arrow in red
\newcommand{\redlink}[3] {
  \node (aiplans circle from) at (#1) {};
  \node (aiplans circle to)   at (#2) {};
  \draw[line width = 1, red, -] (aiplans circle from) to node[midway,above] {$\times$} (aiplans circle to); 
}

% A version of link with the \draw arrow in red
\newcommand{\bendaredlink}[3] {
  \node (aiplans circle from) at (#1) {};
  \node (aiplans circle to)   at (#2) {};
  \draw[line width = 1, red, bend right=45, -] (aiplans circle from) to node[midway,above] {$\times$} (aiplans circle to); 
}
\newcommand{\bendbredlink}[3] {
  \node (aiplans circle from) at (#1) {};
  \node (aiplans circle to)   at (#2) {};
  \draw[line width = 1, red, bend left, -] (aiplans circle from) to node[midway,above] {$\times$} (aiplans circle to); 
}

% A version of link with the \draw arrow in green
\newcommand{\greenlink}[3] {
  \node (aiplans circle from) at (#1) {};
  \node (aiplans circle to)   at (#2) {};
  \draw[line width = 1, green, -] (aiplans circle from) to node[midway,above] {\checkmark} (aiplans circle to); 
}
...
\def\openaction#1#2{
  ...
  \foreach \pre [count=\k] in \aiplans@pres{
    ...
    % Draw the precondition line 
    \draw[aiplans action limb, line width = 1, red] (\aiplans@name/pre/\k) -- (\aiplans@name/pre/start/\k);
    
    % Draw the precondition text
    \node at (\aiplans@name/pre/start/\k) [anchor=east,xshift=.5mm,yshift=1.5mm, text=red, font=\bfseries] {\pre};
  }
  ...
  \foreach \eff [count=\k] in \aiplans@effs {
    ...
    \StrRemoveBraces{\eff}[\efff]
  
    % Draw the line for effect
    \IfStrEq{\eff}{\efff}{%
      \draw[aiplans action limb] (\aiplans@name/eff/start/\k) -- (\aiplans@name/eff/\k);
    }{%
      \draw[aiplans action limb, line width = 1, red] (\aiplans@name/eff/start/\k) -- (\aiplans@name/eff/\k);
    }%
    ...
    \IfStrEq{\eff}{\efff}{%
      ...
      \node at (\aiplans@name/eff/start/\k) [anchor=west,xshift=-.5mm,yshift=1.5mm, text=red, font=\bfseries] {\eff};%
    }%
...
\def\opengreenaction#1#2{
  ...
  \foreach \eff [count=\k] in \aiplans@effs {
    ...
    % Draw the line for effect
    \IfStrEq{\eff}{\efff}{%
      \draw[aiplans action limb, line width = 1, green] (\aiplans@name/eff/start/\k) -- (\aiplans@name/eff/\k);
    }{%
      \draw[aiplans action limb, line width = 1, red] (\aiplans@name/eff/start/\k) -- (\aiplans@name/eff/\k);
    }%
    ...
    % Place the text using a node, correctly positioned
    \IfStrEq{\eff}{\efff}{%
      \node at (\aiplans@name/eff/start/\k) [anchor=west,xshift=-.5mm,yshift=1.5mm, text=green, font=\bfseries] {\eff};%
    }{%
      \node at (\aiplans@name/eff/start/\k) [anchor=west,xshift=-.5mm,yshift=1.5mm, text=red, font=\bfseries] {\eff};%
    }%
...
\def\goalaction#1#2{
  ...
  \foreach \pre [count=\k] in \aiplans@pres{
    ...
    % If k <= 2
    \ifnum\k=2
      \draw[aiplans action limb, line width = 1, green] (\aiplans@name/pre/\k) -- (\aiplans@name/pre/start/\k);
    \else
      \draw[aiplans action limb, line width = 1, red] (\aiplans@name/pre/\k) -- (\aiplans@name/pre/start/\k);
    \fi

    \ifnum\k=2
      \node at (\aiplans@name/pre/start/\k) [anchor=east,xshift=.5mm,yshift=1.5mm, text=green, font=\bfseries] {\pre};
    \else
      \node at (\aiplans@name/pre/start/\k) [anchor=east,xshift=.5mm,yshift=1.5mm, text=red, font=\bfseries] {\pre};
    \fi
